...
const rules = [
    {
        'value': 'basketball has:images lang:en',
        'tag': 'basketball',
    },
    {
        'value': 'football has:images lang:en',
        'tag': 'football',
    },
    {
        'value': 'cricket has:images lang:en',
        'tag': 'cricket',
    },
    {
        'value': 'swimming has:images lang:en',
        'tag': 'swimming',
    },
    {
        'value': 'tennis has:images lang:en',
        'tag': 'tennis',
    },
];
...
function streamConnect(retryAttempt) {
    ...
    stream.on('data', data => {
        try {
            ...
            // Check S3
            const params = { Bucket: bucketName, Key: tags };
            s3.getObject(params) 
            .promise() 
            .then((result) => {
                let arr = [];
                //const resultJSON = JSON.parse(result.Body);
                arr.push(JSON.parse(result.Body));
                arr.push(body);

                const objectParams = { Bucket: bucketName, Key: tags, Body: JSON.stringify(arr)}; 
                s3.putObject(objectParams)
                .promise()
                .then(() => {                
                    console.log(
